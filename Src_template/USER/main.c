...
//	read_pid_from_flash(R_PID_FLASH_ADDR, "R_PID", &r_p, &r_i, &r_d);
	...
	set_time(20000);//2s
	while(1){
		OLED_ShowString(30, 20, "Get Room:", 12, 1);
		OLED_Refresh();
		if(timeout){
			set_time(20000);
			count = get_num(num, 5);
			...
			if(count == 1){
				OLED_ShowNum(84,20, num[0], 3, 12, 1);
				OLED_Refresh();
				break;
			}
		}
	...
	while(medc_scan() != 1){
		OLED_Clear();
		OLED_ShowString(30, 20, "Wait Med", 12, 1);
		OLED_Refresh();
	}
	OLED_Clear();
	...
	OLED_ShowString(60, 0, "cros:", 12, 1);
	OLED_Refresh();
